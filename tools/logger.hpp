...
public:
    Logger(const QString &path)
    {
        ...
        _tags << "DEBUG" << "WARNING" << "CRITICAL" << "FATAL" << "INFO";
    }
    ~Logger()
    {
        _file.close();
    ...
    {
        _stream << QDateTime::currentDateTime().toString("yyyy-MM-dd hh:mm:ss.zzz") << " " <<
            _tags[type] << " " << context.file << ":" << context.line << " " << context.function << " " << msg << Qt::endl;
    }
};