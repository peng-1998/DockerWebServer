...
def build_image():  # 创建公共镜像
    ...
    messenger.send_all(
        {"type": "image", "data": {"image": attrs["image"], "opt": "pull"}}
    )  # 非阻塞，需要自己监听是否完成拉取
    db.insert_image(
        {
            "imagename": attrs["image"],
            "init_args": attrs["init_args"],
...
def delete_image():
    ...
    messenger: BaseServer = current_app.config["messenger"]
    messenger.send_all(
        {"type": "image", "data": {"image": image_name, "opt": "remove"}}
    )  # 非阻塞，需要自己监听是否完成删除
    return make_response({"success": True}, 200)
...
def allcontainer(machine_id):
    db: BaseDB = current_app.config["db"]
    containers = db.get_container(
        search_key={"machine_id": machine_id},
        return_key=[
            "showname",
            "userid",
            "imageid",
    ...
    for c in containers:
        c["username"] = db.get_user(
            search_key={"id": c["userid"]}, return_key=["account"]
        )[0]["account"]
        c["imagename"] = db.get_image(
            search_key={"id": c["imageid"]}, return_key=["imagename"]
        )[0]["imagename"]
