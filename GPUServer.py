...
if __name__ == "__main__":
    ...
    def pull_image(image: str):
        for _ in range(3):
            res = docker_controller.pull_image(image)
            if isinstance(res, Image):
                messenger.send(
                    {
                        "type": "image",
                        "data": {"image": image, "opt": "pull", "status": "success"},
                    }
                )
                break
    ...
    def remove_image(image: str):
        res = docker_controller.remove_image(image)
        if res:
            messenger.send(
                {
                    "type": "image",
                    "data": {"image": image, "opt": "remove", "status": "success"},
                }
            )
        else:
            messenger.send(
    ...
    def data_handler_image(data: dict):
        threading.Thread(target=image_funcs[data["opt"]], args=(data["image"])).start()
    ...
    def data_handler_container(data: dict):
                ...
                messenger.send(
                    {
                        ...
                        "data": {
                            ...
                            "status": "success",
                            "original_data": data["create_args"],
                        },
                    }
                )
            else:
        ...
        if data["opt"] == "commit":
            try:
                res: Image = container.commit(
                    tag=data["account"] + "/" + data["image_name"] + ":" + "latest"
                )
    ...
    def connect_handler():
        ...
        container_data = [
            {"name": container.name, "status": container.status == "running"}
    ...
    configs = yaml.load(
        open("configs/GPUServerConfig.yaml", "r", encoding="utf-8"),
    ...
    messenger: BaseClient = Messenger_Class(
